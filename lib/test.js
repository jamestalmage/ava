...
var assert = shared.assert;
var defaultAssert = require('./default-assert');

if (!assert) {
	shared.assert = assert = require('./assert-support')([require('./default-assert-plugin')]);
}
...
function Test(title, fn) {
	...
	defaultAssert.is(typeof fn, 'function', 'you must provide a callback');
...
assert.methods.forEach(function (el) {
	Test.prototype[el] = function () {
		var self = this;

		try {
			var fn = assert.object[el].apply(assert.object, arguments);
...
Test.prototype._capt = assert.object._capt;
Test.prototype._expr = assert.object._expr;
...
Test.prototype.run = function () {
	...
	if (isPromise(ret)) {
		ret
			...
			.catch(function (err) {
				self.assertError = new assert.object.AssertionError({
					actual: err,
...
Test.prototype.end = function (err) {
	if (err) {
		this.assertError = new assert.object.AssertionError({
			actual: err,
